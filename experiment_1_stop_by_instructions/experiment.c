...
//citing https://gist.github.com/willb/14488/80deaf4363ed408a562c53ab0e56d8833a34a8aa for callname function
//callname function converts system call numerals to string names
...
static int callback(struct dl_phdr_info *info, size_t size, void *data)
{
   ...
   for (j = 0; j < info->dlpi_phnum; j++)
         printf("\t header %2d: base address=%10p offset=%10p\n", j,
                (void *) info->dlpi_addr, (void*)info->dlpi_phdr[j].p_vaddr);
    return 0;
}

//this would make a thread to attach to the pthread in the testing program
//is canceled, just attached child to the parent thread
/* void* attachtochild(void* tid_ptr){ */

/*   long long tid = (long long)tid_ptr; */
/*   //ptrace(PTRACE_SETOPTIONS, child, NULL, PTRACE_O_TRACEFORK | PTRACE_O_TRACEVFORK | PTRACE_O_TRACECLONE); */
/*   //ptrace(PTRACE_GETEVENTMSG, child, NULL, &threadchild); */
           
/*   //ptrace(PTRACE_SINGLESTEP, child, NULL, NULL); */
/*   while(1){ */
/*     struct user_regs_struct regs; */
/*     ptrace(PTRACE_GETREGS, tid, NULL, &regs); */
/*     printf("The child %lld  made a system call %s\n", tid, callname(regs.orig_rax)); */
/*     getchar(); */
      ...
      if (strcmp(command, "by_sys_call") == 0)  {
        ...
        /*if(regs.orig_rax == SYS_CLONE){  
          ...
          if(syscall == 0){
            ...
            long eax = ptrace(PTRACE_PEEKUSER, current, NULL, NULL);
        ...
        */
        ...
        printf("instruction: RIP: %p RSP %p\n", (void*)regs.rip, (void*)regs.rsp); /* instruction pointer */

        dl_iterate_phdr(callback, NULL);
        
        //convert to a file/line format?
        //execlp(addr2line, -e, --(void*)regs.rip, (char*) NULL);
        getchar();
